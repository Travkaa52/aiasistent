...
async def log_message(user_id: int, text: str, direction: str = "incoming"):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO messages (user_id, message_text, direction) VALUES (?, ?, ?)",
...
async def add_manager(telegram_id: int, username: str, full_name: str, role: str = "manager"):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            """INSERT OR REPLACE INTO managers (telegram_id, username, full_name, role)
               VALUES (?, ?, ?, ?)""",
...
async def is_manager(telegram_id: int):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        async with db.execute(
            "SELECT id FROM managers WHERE telegram_id=?", (telegram_id,)
        ) as cursor:
...
async def add_chat(chat_id: int, title: str, chat_type: str):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            """INSERT OR REPLACE INTO chats (chat_id, title, chat_type)
               VALUES (?, ?, ?)""",
...
async def add_autoreply(keyword: str, reply_text: str, reply_type: str = "text",
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            """INSERT INTO autoreply (keyword, reply_text, reply_type, file_id, caption)
               VALUES (?, ?, ?, ?, ?)""",
...
async def get_autoreplies():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM autoreply") as cursor:
            return await cursor.fetchall()
...
async def add_spam_filter(filter_type: str, value: str):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO spam_filter (filter_type, value) VALUES (?, ?)",
            (filter_type, value)
        )
        await db.commit()
...
async def get_spam_filters():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM spam_filter") as cursor:
            return await cursor.fetchall()
...
async def close_ticket(ticket_id: int, manager_id: int):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            """UPDATE support_tickets SET status='closed', manager_id=?, closed_at=CURRENT_TIMESTAMP
               WHERE id=?""",
